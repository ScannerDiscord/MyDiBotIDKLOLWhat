...
class Tanki:
  ...
  async def supplies (self , ctx ,user:str):
    url = "http://ratings.tankionline.com/get_stat/profile/?user={}&lang=en".format(user)
    async with aiohttp.get(url) as r:
        if r.status == 200:
            response = (await r.json())["response"]["suppliesUsage"]
            damage = response["5"]["usages"]
            print(damage)
        else:
            print(":C")

  @commands.command(pass_context=True)
  async def dropgold(self ,ctx):
        embed=discord.Embed(description="**{} has dropped the gold box** <:gold:419067513712672769>".format(ctx.message.author.name),color=0xff8000)
        x = await self.bot.say(embed=embed)
        msg = await self.bot.wait_for_message(channel=ctx.message.channel)
        await self.bot.delete_message(x)
        embed=discord.Embed(description="**{} has taken the gold box** <:gold:419067513712672769>".format(msg.author.name),color=0xff8000)
        await self.bot.say(embed=embed)

  @commands.command(pass_context=True , aliases=["xp"])
  async def exp(self , ctx , user:str):
    url = "http://ratings.tankionline.com/get_stat/profile/?user={}&lang=en".format(user)
    async with aiohttp.get(url) as r:
      if r.status == 200:
